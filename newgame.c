...
{
    system("color 07");
    int array[size][size], array1[size][size], array2[size][size], score1=0, score2=0, turnArray[60];int undos=0;int  stor[30], stoc[30];
    setToZero(size,array);int removedline[20];
    setToZero(size,array1);
    setToZero(size,array2);
    char active[size][size];
    for(int i=0;i<size;i++)
    ...
    while(noMoves>=0)
    {
        ...
        { int removedline;
            ...
            if(turn==0)
            {
                ...
                {
                    redo2(score1, score2,counter, size, array, array1, array2, active, stor, stoc, turn, passive, removedline, undos);undos--;counter++;noMoves++;goto label;
                }}
                //undo for player 2
                if(row==-1 && col==-1)
                {
            ...
            else
            {
                ...
                {
                    redo2(score1, score2,counter, size, array, array1, array2, active, stor, stoc, turn, passive, removedline, undos);undos--;noMoves--;counter++;goto label2;
                }
