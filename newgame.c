        ...
        if(noMoves>0)
        {
            if(turn==0)
            {
                ...
                if(undos == 0){printf("No moves to redo");}
                    /*else if(noMoves%2!=0){printf("Please undo 1 move to be able to redo" );goto reread;}*/else{

                    {
                        redo1(&score1, &score2, counter, stor, stoc, size, array2, array1, array, active, turn, removedlines, undos, passive );
                        if(undos==1){noMoves--;undos--;counter++;}else{noMoves=noMoves-2;undos=undos-2;counter=counter+2;}
